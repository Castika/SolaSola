import yaml
from flask import Flask, jsonify, render_template, request, send_from_directory, make_response, url_for
...
# Parse command-line arguments at the module level. This is safe because the
# app is run with `use_reloader=False`, preventing the script from running twice.
parser = argparse.ArgumentParser(description="Run SolaSola Flask App.")
parser.add_argument('--no-debug', action='store_true', help="Run in production mode without debug features.")
parser.add_argument('--keep-models-cached', action='store_true', help="Keep AI models in memory after a task is finished for faster subsequent processing.")
...
# Define fixed paths inside the container. These are mounted as volumes from the
# host machine by the startup script (dog.sh), pointing to user-configured
# directories (e.g., ~/Documents/SolaSola_Music).
...
# This log filter removes noisy, non-critical warnings from libraries (e.g., "Tensorflow
# is not installed") to make the server console log cleaner and easier to debug.
...
def offline_page():
    ...
    # This page SHOULD be cached by the browser so it can be displayed when the server is down.
...
def get_user_config():
    """
    Provides user-defined configuration from a JSON file.
    This allows users to override default frontend constants without editing source code.
    """
    ...
    return jsonify({}) # Return empty object if no custom config exists
...
def run_health_checks():
    ...
    if not shutil.which('ffmpeg'):
        ...
        checks['ffmpeg']['message'] = 'FFmpeg executable not found in PATH. Audio processing will fail.'
        overall_status = 'degraded'

    if not shutil.which('midi2abc'):
        checks['abcmidi']['status'] = 'missing'
        checks['abcmidi']['message'] = 'midi2abc (from abcmidi) not found in PATH. ABC conversion will fail.'
        overall_status = 'degraded'
...
def health_check():
    ...
    response = make_response(jsonify({'status': status, 'checks': checks}), http_status_code)
...
def models_status():
    ...
    try:
        ...
        logging.error(f"Error checking language model status: {e}")
...
def manage_model():
    ...
    if action == 'install':
        if not INSTALL_LOCK.acquire(blocking=False):
            ...
            return jsonify({"status": "waiting", "message": "Another installation is already in progress."})
        ...
        try:
            ...
            traceback.print_exc()
            return jsonify({'error': 'An internal server error occurred during model installation.'}), 500
...
def start_processing():
    ...
    try:
        ...
        thread = threading.Thread(target=process_task_wrapper, args=(
            task_id, temp_dir, classified_files, processing_mode, demucs_model, original_music_filenames, raw_form_data.get('display_title'),
            cli_args.keep_models_cached, BASE_OUTPUT_DIR, raw_form_data, VERSION_INFO
        ))
        thread.daemon = True
        thread.start()
