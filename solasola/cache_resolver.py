...
from .task_manager import TASKS, log_to_ui

def _validate_processing_cache(directory: Path) -> bool:
    """Validates a processing cache directory against its manifest."""
    ...
    try:
        ...
        # A cache is only considered valid if it contains the expected type of output files
        # (e.g., .wav for stems), not just error logs or other intermediate artifacts.
        # This prevents the system from incorrectly reusing a failed processing step.
        ...
        # Verify that every file listed in the manifest exists and has the correct size.
...
def _safe_copy_tree(source_dir: Path, dest_dir: Path):
    """
    Recursively copies a directory. Unlike shutil.copytree, it won't fail
...
class CacheResolver:
    """
    Handles all cache lookups, validation, and copying for a given processing task.
    This centralizes cache logic, simplifying the main processing functions.
    """
    def __init__(self, task_id: str, base_output_dir: Path, fingerprint: str, result_dir: Path):
        self.task_id = task_id
        # This is the path *inside* the container, which is mounted as a volume
        # from the host machine (e.g., ~/Documents/SolaSola_Music).
    ...
    def _find_candidate_folders(self) -> list[Path]:
        """Finds all folders that match the fingerprint, sorted by latest first."""
        candidates = []
        ...
        # Sort by folder name descending, which corresponds to latest timestamp
    ...
    def resolve(self, asset_type: str) -> dict:
        """
        Resolves an asset (e.g., 'stems', 'midi').
        If a valid cache is found, it copies the files and returns a USE_EXISTING instruction.
        Otherwise, it returns a CREATE_NEW instruction.
        """
        ...
        for candidate_folder in self.candidate_folders:
            ...
            if source_path.is_dir() and _validate_processing_cache(source_path):
                ...
                log_to_ui(self.task_id, message, icon, type='info')
                ...
                try:
                    ...
                    # Record provenance and return instruction
        ...
        if asset_type == 'stems' and self.task_id in TASKS and len(TASKS[self.task_id].get('input_files', {}).get('audio', [])) > 1:
            ...
            message, icon = log_messages.get(asset_type, (f"Processing {asset_type} from scratch...", "info"))

            log_to_ui(self.task_id, message, icon)
    ...
    def get_file_hash(file_path: str) -> str:
        """Computes the SHA256 hash of a file to use as a unique cache key."""
    ...
    def write_manifest_for_step(self, asset_type: str):
        """Creates a manifest file for a specific processing step's output directory."""
        directory = self.result_dir / asset_type
        
        # Do not write a manifest if no valid output files were created for this step.
        # This is a crucial step to prevent caching directories that only contain error logs.
        expected_extensions = {
            'stems': '.wav',
            'midi': '.mid',
            'abc_files': '.abc',
