...
from solasola import song_analyzer

# Import from the new task manager
from solasola.task_manager import TASKS, update_detailed_status, log_to_ui, check_for_cancellation, InterruptedError
...
def _validate_and_get_duration(task_id, audio_files: list, midi_files: list) -> float:
    ...
    if audio_files:
        ...
        log_to_ui(task_id, "Validating audio file durations...", "rule")
        try:
            ...
            if len(durations) > 1 and (max(durations) - min(durations) > 1.5):
                ...
                log_to_ui(task_id, f"Error: Files have significantly different durations ({duration_str}). They appear to be unrelated songs.", "error")
    ...
    if midi_files:
        ...
        log_to_ui(task_id, "Calculating duration from MIDI files...", "timer")
        try:
...
def _run_genre_classification(task_id, audio_path, temp_dir):
    ...
    if not genre_model_status or not genre_model_status.get('installed'):
        ...
        log_to_ui(task_id, "Genre analysis model is not installed. Skipping. You can install it from the 'Manage Models' menu.", "info")
        return []
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            log_to_ui(task_id, "Genre analysis failed and was skipped. See server logs for details.", "error", type='warning')
            return []
        ...
        log_to_ui(task_id, "Genre detection skipped (model not available).", "info")
        return []
    except Exception as e:
...
def _process_song(task_id, title, files, temp_dir, device, models, mode, base_progress, total_songs, song_num, cache_resolver, metadata_generator, audio_duration=0.0):
    ...
    if audio_duration == 0 and mode == 'lyrics_only' and files.get('lyrics'):
        ...
        log_to_ui(task_id, "No music file provided. Using default duration for lyrics split.", "timer")
    ...
    try:
        ...
        if files.get('audio'):
            ...
            if len(files['audio']) > 1:
                ...
                log_to_ui(task_id, "Creating temporary mix for analysis...", "blender")
                created_path = create_mix_audio(files['audio'], str(mixed_audio_path))
                if created_path:
                    audio_for_analysis = created_path
                else:

                    log_to_ui(task_id, "Could not create a temporary mix. Analysis will be based on the first stem only.", "warning")
    ...
    if mode == 'abc' and 'audio_for_analysis' in locals():
        try:
            ...
            log_to_ui(task_id, f"Pre-analysis failed for '{title}': {e}", "error", type='error')
            traceback.print_exc()
    ...
    if mode == 'abc':
        try:
            ...
            log_to_ui(task_id, f"Audio processing failed for '{title}': {e}", "error", type='error')
    ...
    if mode == 'abc' and midi_paths_for_abc:
        ...
        if abc_instruction['action'] == 'USE_EXISTING':
            ...
            try:
                ...
                log_to_ui(task_id, f"ABC score generation failed for '{title}': {e}", "error", type='error')
                traceback.print_exc()
    elif mode == 'abc':
    ...
    if mode == 'abc' and midi_paths_for_abc:
        ...
        if mix_midi_path:
            ...
            log_to_ui(task_id, "Could not create a combined Mix. Skipping final MIDI analysis.", "warning")
    ...
    try:
        ...
        for title, files in songs_to_process.items():
            try:
                ...
                # Use a pipe separator to show a short message in the toast and a detailed one in the log.
                toast_message = "Results will be saved to..."
                log_to_ui(task_id, f"{toast_message}|This folder: '{result_dir_name}'", "folder_open", type='success')
                ...
                log_to_ui(task_id, f"Failed to process '{title}': {song_e}", "error", type='error')
        ...
        log_to_ui(task_id, str(e), "error", type='error')
    finally:
        ...
        if processing_mode == 'abc':
            if not keep_models_cached:
                ...
                log_to_ui(task_id, "Releasing AI models from memory...", "autorenew")
                ...
                log_to_ui(task_id, "Keeping AI models in memory for faster processing.", "memory")
...
def process_lyrics(task_id, files, audio_duration=0):
    ...
    if not files.get('lyrics'):
        ...
        log_to_ui(task_id, "No lyrics file provided. Lyrics will not be included in the result.", "info")
    ...
    log_to_ui(task_id, "Distributing lyrics evenly across the song's duration.", "linear_scale")
