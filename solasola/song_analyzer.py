    ...
    if audio_path:
        try:
            ...
            try:
                ...
                print(
                    f"  -> [INFO] Song structure analysis failed and was "
                    f"skipped: {e}")
                traceback.print_exc()
...
def create_song_profile(result_data: dict, audio_duration: float) -> dict:
    ...
    # Add the raw chord analysis results to the profile dictionary. This makes
    # them available to the backend for saving to files.
    if 'detailed_sync_chords_srt' in result_data: # noqa
        profile['detailed_sync_chords_srt'] = result_data['detailed_sync_chords_srt'] # noqa
    if 'simple_sync_chords_srt' in result_data: # noqa
        profile['simple_sync_chords_srt'] = result_data['simple_sync_chords_srt'] # noqa
    if 'chord_grid_text' in result_data: # noqa
        profile['chord_grid_text'] = result_data['chord_grid_text'] # noqa
