...
def _parse_srt_time(time_str: str) -> float:
    """Converts an SRT time string (HH:MM:SS,ms) to a total number of seconds."""
        ...
        for block in blocks:
            ...
            if len(lines) >= 2 and '-->' in lines[1]:
                ...
                if time_match:
                    ...
                    text = " ".join(lines[2:])
                    lyrics.append({'start': start_sec, 'end': end_sec, 'text': text})
        return lyrics
...
def create_srt_from_txt_file(file_path: str, encoding: str, total_duration: float) -> tuple[str, str, list] | tuple[None, None, None]:
    """
