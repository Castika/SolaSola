...
class SSEManager:
    ...
    def unsubscribe(self, client_queue):
        ...
        try:
            ...
            # This can happen if a client disconnects and the generator tries to
    ...
    def broadcast(self, message: dict):
        ...
        for client_queue in list(self.clients):
            try:
                ...
                # If putting a message into a queue fails, it might mean the client
                # is gone. It's safer to let the `stream` generator handle the
    ...
    def stream(self):
        ...
        try:
            # Send an initial connection confirmation event. This immediately triggers
            # the 'onopen' event on the client and prevents an initial delay while
            # the queue waits for its first message.
            yield ":connected\n\n"
            while True:
                try:
