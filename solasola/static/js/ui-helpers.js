...
export function truncateFilename(filename, maxLength = 40) {
    // --- FIX: Add a safeguard to handle undefined or null inputs ---
...
function openModal(onCloseCallback) {
    if (!solaModal) return;
    // Store the callback on the element itself to be retrieved by closeModal
...
export function closeModal() {
    if (!solaModal) return;
    // If an onClose callback was stored, execute it and then clear it.
    ...
    if (notificationContainer) {
        ...
        // Reset the z-index so it's controlled by the stylesheet again.
...
export function showModal({ title, bodyContent, footerButtons = [], onClose = null }) {
    ...
    footerButtons.forEach(btn => {
        ...
        button.className = 'modal-footer-btn'; // Common class
...
function addToast(data) {
    if (visibleToasts.length >= TOAST_CONFIG.MAX_TOASTS) {
    ...
    const toastObject = {
        ...
        height: 0,
        // Use the duration from the log object, or fall back to the default.
    ...
    // If this is the ONLY toast, it's on the 1st floor, so start its timer
...
function removeToast(toastId) {
    ...
    // Check if a new toast can be added from the queue to fill the top spot
    processToastQueue();
...
function createToastElement(data, id) {
    ...
    const iconSpan = document.createElement('span');
    iconSpan.className = 'material-symbols-outlined';
    iconSpan.textContent = data.icon; // Use textContent for safety
...
function repositionToasts() {
    ...
    visibleToasts.forEach((toast, index) => {
        ...
        // Apply cascade delay for the transition
...
export function clearAllToasts() {
    // 1. Clear the queue of upcoming toasts.
    ...
    // 2. Immediately remove all toasts currently visible on screen.
...
function logNotificationToHistory(logObject) {
    ...
    // Enforce a maximum number of log entries to prevent performance issues.
...
function refreshLogView(showToast = true, logData = null) {
    ...
    if (!logData || logData.length === 0) {
        ...
        // Use the provided or freshly fetched log data, not the global variable.
        [...logData].reverse().forEach(log => {
            ...
            const displayMessage = log.message; // The message is now pre-formatted for the log.
            // Use the full detailed message for the log, but truncate it if it's excessively long.
