...
class DemucsProgressParser:
    """
    Parses the real-time stdout of the Demucs subprocess to extract structured progress updates.
    This encapsulates the fragile parsing logic, separating it from the main processing flow.
    """
    ...
    def parse_line(self, line: str) -> dict | None:
        ...
        Returns:
            A dictionary like {'stage': 4, 'sub_stage': 1, 'progress': 10, 'message': '...'} or None.
        """
        stripped_line = line.strip()
        if not stripped_line:
            return None

        # Use logging.debug to show raw Demucs output only when in debug mode.
        # This keeps the production log clean while allowing for detailed debugging.
        ...
        match = self.progress_pattern.search(stripped_line)
        if match and self.phase == 'separate':
            ...
            message = (f"Ensemble processing ({min(sub_stage_index, self.num_models)}/{self.num_models}) - {progress}%" 
                       if self.is_ensemble else f"Separating instruments ({progress}%)")
            self.last_reported_progress = progress
            return {'stage': 4, 'sub_stage': sub_stage_index, 'progress': progress, 'message': message}
