...
def get_current_state(ai_models_dir: Path) -> dict:
    """Scans the target directory and returns a dictionary of file paths and their metadata."""
    state = {}
    for root, dirs, files in os.walk(ai_models_dir, topdown=True):
        # Prune excluded directories to avoid scanning them
        dirs[:] = [d for d in dirs if not is_path_excluded(Path(root) / d, ai_models_dir)] # Use the centralized exclusion logic.
        
        for name in files:
            file_path = Path(root) / name
            if not is_path_excluded(file_path, ai_models_dir): # Use the centralized exclusion logic.
                try:
                    # Use lstat() to get info about the symlink itself, not its target file.
...
def start_watching(task_id: str):
    ...
    ai_models_dir = get_ai_models_dir() # FIX: Use the imported utility function.
    ...
    print(f"  -> Initial state with {len(initial_state)} files saved to temporary state file.")
...
def stop_watching_and_register(task_id: str, model_name: str):
    ...
    ai_models_dir = get_ai_models_dir() # FIX: Use the imported utility function.
    manifest_dir = get_manifest_dir(ai_models_dir) # FIX: Use the imported utility function.
    state_file = _get_state_file_path(task_id)

    if not state_file.exists():
        print("  -> ERROR: Initial state file not found. Cannot determine changes.")
        return
    ...
    if not new_files:
        ...
        for file_path_str in new_files:
            ...
            # --- FIX: Use lstat again to get the correct symlink size ---
        ...
        # --- FIX: Adhere to the unified manifest format, including the full file list ---
        manifest_data = {
            "name": f"Demucs - {model_name}",
            "repo_id": model_name, # For Demucs, the name is the unique ID
        ...
        # Use a consistent, predictable filename based on the model name
        manifest_filename = f"demucs_{model_name}.json" # e.g., demucs_htdemucs_ft.json
    ...
    try:
        ...
        print(f"  -> Cleaned up temporary state file.")
    except OSError as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SolaSola Demucs Download Watcher.")
    parser.add_argument("--action", required=True, choices=['start', 'stop'], help="The action to perform.")
    parser.add_argument("--task-id", required=True, help="The unique ID of the processing task.")
    parser.add_argument("--model-name", help="The name of the Demucs model being used (required for 'stop').")
